...
function MintNFTPage(props) {
  ...
  const [tokenImageCollection, setTokenImageCollection] = React.useState(null);
  ...
  const handletokenImageCollection = files => {
    setTokenImageCollection(files);
  }
  ...
  const handleMint = async () => {
    ...
    if((parseInt(nextTokenCount) - parseInt(previousTokenCount)) === parseInt(tokenCount)) { // success
      ...
      if(tokenImageCollection) {
        Array.from(tokenImageCollection).forEach(item => {
          ...
          {!accountError ? (
            <form>                       
              ...
              <GridContainer>
                ...
                <GridItem xs={12} sm={7}>
                  <ImageMultipleUpload
                    onChange={handletokenImageCollection}
                    addButtonProps={{
                      color: "rose",
                      round: true
                    }}
